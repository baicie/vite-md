...
import { version } from '../packages/plugin-md/version'
...
async function main() {
  ...
  consola.debug(chalk.yellow('Updating package.json for @baicie/vite-md'))

  const pkgs = Object.fromEntries(
    (await getWorkspacePackages()).map(pkg => [pkg.manifest.name!, pkg]),
  )

  const BaicieCli = pkgs['@baicie/vite-md']
  ...
  consola.success(chalk.green(`package @baicie/vite-md updated successfully to version ${version}`))
}
